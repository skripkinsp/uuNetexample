...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        error = x.dot(self.w) - y
        loss = (1 / y.size) * error.T.dot(error)
        return loss
        """
...
van = VanillaGradientDescent(dimension)
print(van.calc_loss(x, y))
for i in range(100000000):
    van.update_weights(van.calc_gradient(x, y))
    print(van.calc_loss(x, y))
